...
USE_OLD_SHADER = bpy.app.version < (3, 5, 0)
if USE_OLD_SHADER:
    import bgl
...
from ...utils.edge_topology import div_set
...
class RARA_OT_Model_SlideEdge(bpy.types.Operator):
    bl_idname = "rara.model_slide_edge"
    bl_label = "滑移边线"
    bl_description = "精准的滑移选中的边线【RARA修改版】"
    bl_options = {"REGISTER", "UNDO", 'GRAB_CURSOR', 'BLOCKING'}

    prop_plen: FloatProperty(name="滑移尺寸", description="定义滑动尺寸", default=0.04, step=0.5, unit='LENGTH')
    prop_select: BoolProperty(name="选择结束边", description="选择结束边", default=False,)
    prop_original_select: BoolProperty(name="选择起始边", description="选择起始边", default=False,)
    ...
    def addline(self, p1, p2):
        self.gui_lines.append(p1)
        self.gui_lines.append(p2)

    def draw_3d(self, context, event=None):
        self.draw_line(self.gui_lines, (1, 1, 0, 1), True, True, 1)
        self.draw_line(self.gui_lines2, (1, 1, 1, 1), True, True, 4)
        if hasattr(self, '_hud_text'):
            draw_hud_text(self._hud_text, context)

    def draw_line(self, points, color, blend=False, smooth=False, width=1):
        if USE_OLD_SHADER:
            self.draw_line_gl(points, color, blend=blend, smooth=smooth, width=width)
            return
    ...
    def draw_line_gl(self, points, color, blend=False, smooth=False, width=1):
        if len(points) == 0:
            return
        if blend:
    ...
    def draw_handle_add(self, arg):
        self.handle3d = bpy.types.SpaceView3D.draw_handler_add(
            self.draw_3d, arg, 'WINDOW', 'POST_VIEW')

    def draw_handle_remove(self):
        if self.handle3d != None:
            bpy.types.SpaceView3D.draw_handler_remove(
                self.handle3d, 'WINDOW')
    ...
    def get_angle(self, n1, n2, sn=None):
        m1 = n1.normalized()
        m2 = n2.normalized()
        if sn is None:
            return m1.angle(m2)
    ...
    def on_move(self, context, event):
        ...
        self._hud_text = self.update_text()
    ...
    def draw_point(self, p1):
        self.gui_lines += [p1 + Vector((0.02, 0, 0)), p1 - Vector((0.02, 0, 0))]
        self.gui_lines += [p1 + Vector((0, 0.02, 0)), p1 - Vector((0, 0.02, 0))]

    def update_text(self):
        return "滑移尺寸: " + self.format_length_with_unit(self.prop_plen) + " | 左键/回车: 确认 | Esc: 取消"

    def modal(self, context, event):
        context.area.tag_redraw()
        if event.type == 'LEFTMOUSE' and event.value == 'PRESS':
            self.draw_handle_remove()
            return {'FINISHED'}
            ...
            self.bm2.free()
            return {'CANCELLED'}
        elif event.type in {'RETURN', 'ENTER', 'RET', 'NUMPAD_ENTER'} and event.value == 'PRESS':
            self.draw_handle_remove()
            return {'FINISHED'}
        return {'PASS_THROUGH'}
    ...
    def invoke(self, context, event):
        if context.edit_object:
            ...
            self.px = 0
            self.py = 0
            self.draw_handle_add((self, context))
            self._hud_text = self.update_text()
            self.process(context)
    ...
    def format_length_with_unit(self, meters_value):
        ...
        if length_unit == 'METERS':
            value = meters_value / scale_length
        elif length_unit == 'CENTIMETERS':
            value = meters_value * 100.0 / scale_length
        elif length_unit == 'MILLIMETERS':
            value = meters_value * 1000.0 / scale_length
        elif length_unit == 'KILOMETERS':
            value = meters_value / 1000.0 / scale_length
        elif length_unit == 'INCHES':
            value = meters_value / 0.0254 / scale_length
        elif length_unit == 'FEET':
            value = meters_value / 0.3048 / scale_length
        elif length_unit == 'MILES':
            value = meters_value / 1609.344 / scale_length
        else:
            value = meters_value
        symbol, precision = unit_config.get(length_unit, ("", 3))
